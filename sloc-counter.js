...
const commentPattern = commentDelimiters.map(x => `^\\s*${x}`).join('|');
const slocPattern = new RegExp(`${allWhitespacePattern}|${commentPattern}`);
...
Observable.from(lines)
  ...
  .count()
  .subscribe(console.log.bind(console, 'sloc:'));
